...
const phone = '0574 198 0036'
const tel = '+3905741980036'
...
function trackCall(placement: string) {
  ...
  try {
    ...
    const payload = { evento: 'click_telefono', fonte: 'sponsorizzata_landing', page_type: 'landing_call_only', citta: 'Prato', pagina: '/richiesta-prato', telefono: tel, telefono_label: phone, placement, url: window.location.href, dominio: window.location.hostname, timestamp: new Date().toISOString(), ...campaign }
...
const benefits = [
  { icon: 'clock' as const, title: 'Meno passaggi. Più rapidità.', text: 'Parli direttamente con il riferimento per Prato. Descrivi dove sei e cosa è successo: il recupero parte da qui.' },
  ...
  { icon: 'pin' as const, title: 'Il soccorso adatto alla zona.', text: 'Prato e provincia: comunica via, comune o punto di riferimento. Concorda tempi e destinazione al telefono.' },
]
const faqs = [
...
export default function PratoCall() {
  return <div className="bc-page">
    <header className="bc-header bc-wrap"><img src="/images/viasos-logo-header-cropped.webp" alt="ViaSOS" width="180" height="70" /><div className="bc-header-zone"><Icon name="pin" /><span>Prato e provincia<strong>Soccorso stradale 24 ore</strong></span></div><a className="bc-header-phone" href={`tel:${tel}`} onClick={() => trackCall('header')}><Icon />{phone}</a></header>
    <main>
      <section className="bc-hero"><div className="bc-wrap bc-hero-grid">
        <div className="bc-intro"><p className="bc-eyebrow"><span /> SOCCORSO STRADALE · PRATO</p><h1>Auto ferma?<br />Il prossimo passo<br /><em>è una chiamata.</em></h1><div className="bc-price-focus"><span className="bc-price-kicker">IL VANTAGGIO È NEL PREZZO</span><p><strong>Con noi spendi meno.</strong><span>Costi inferiori rispetto agli altri carroattrezzi in zona <b>Prato e provincia.</b></span></p></div><div className="bc-promises"><span><Icon name="check" />Preventivo prima dell’uscita</span><span><Icon name="check" />Nessun obbligo di accettare</span></div><div className="bc-route" aria-hidden="true"><span className="bc-route-pin">⌖</span><span className="bc-route-line" /><span className="bc-route-truck">↗</span><span>PRATO<br /><b>Partiamo da dove sei.</b></span></div></div>
        <div className="bc-contact"><div className="bc-contact-top"><span className="bc-availability"><i /> ASSISTENZA TELEFONICA 24/7</span></div><h2 className="bc-available-title">Carroattrezzi <strong>disponibile</strong> al 100% in questo momento.</h2><CallButton placement="hero" /><p className="bc-call-note">Un contatto diretto. Nessun modulo da compilare.</p><div className="bc-contact-bottom"><Icon name="shield" /><p><strong>Chiedi il preventivo. Poi decidi.</strong><span>Prezzo e tempi del mezzo si confermano al telefono.</span></p></div></div>
      ...
      <section className="bc-final bc-wrap"><div><p className="bc-eyebrow">PRATO E PROVINCIA · 24 ORE</p><h2>Fermo con il veicolo?<br /><em>Facciamo il prossimo passo.</em></h2><p>Chiama, spiega il problema e chiedi il tuo preventivo.</p></div><CallButton placement="closing" /></section>
    </main>
